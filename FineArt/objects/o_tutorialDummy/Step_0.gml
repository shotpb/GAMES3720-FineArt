...
{
	if (!instance_exists(o_popupE) and !instance_exists(o_MainUI))
	...
	if (keyboard_check_pressed(ord("E")) && !instance_exists(o_MainUI))
	{
		...
		var UI = instance_create_layer(view_get_xport(0) + (view_get_wport(0) / 2), view_get_yport(0) + (view_get_hport(0) / 2) - 50, "UI_Elements", o_dialogueSpecialTutorial);
		with (UI)
		{
			my_txt = other.myDialogue;
			canSell = other.can_sell;
			speaker_order = other.order;
